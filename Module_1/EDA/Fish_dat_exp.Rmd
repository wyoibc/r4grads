...
Even with some transparency, if we plotted all seven of our species, this would get hard to interpret pretty quickly. One option for plotting multiple is to use box plots. Personally, I prefer violin plots from ggplot, but we'll start with box plots for now
...
This looks ok. There is a lot of customization we could do with this, but again, we won't get into that here. We can see here that it looks like we have some major differences in body sizes across the different fish species. Let's test that out using ANOVA. 

...
Lists are what they sound like - lists of things. Note that they can be much more complicated than they sound, however. Lists can contain almost anything as an element of a list. They can be simple lists of values:
...
* Note that while you'll occasionally create objects from scratch, but most of the time you'll reading data into R from a file, we'll get into that in the next session.
...
Let's take the `year` dataframe and write that out to a csv file. csv, or comma separate values files, are basically a simple type of spreadsheet that can be easily read into R and other programming languages. This is one of of the standard formats to keep data in for this reason.
...
Bioconductor is third repository that is commonly used in biology, which contains a number of bioinformatic and genomic oriented packages. Like installing from Github, we need to install a package from CRAN that contains functions that allow us to install packages from Bioconductor:
...
- getting help - cover in the next one

 - staying current - biggest issues are old R, old packages

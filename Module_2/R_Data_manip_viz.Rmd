...
Now that we have a `coho_data` object, make a plot for this species that has `Fork.length..cm.` on the x axis with `del15N` on the y axis, and points sized by `Weight..g.`. Change the color of the points to a color of your preference.
...
There is a lot more customization we can do, and we can create many other types of plots. We'll explore more plotting options in the next sessions.

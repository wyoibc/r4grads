...
So far, we've done all of the manipulation using base R function. We can also do a lot of fancy data manipulation using the [Tidyverse](https://www.tidyverse.org/) set of R packages, which all share some general principles in their architecture. 
...
Pipes allow you to chain (or "pipe") together multiple functions into a single long command. If you're familiar with the Linux/Unix command line, this is done with the `|` character. R uses that for some other purposes, so Tidyverse instead introduces the pipe as `%>%`. I honestly think this is incredibly unwieldy, but here we are.
